...
export default function RewardsMarketplace({ wallet, balance, onRedemption }: RewardsMarketplaceProps) {
  ...
  const handleRedeem = async (reward: Reward) => {
    if (balance < reward.eco_cost) {
      toast({
        title: '$EC0 insuficiente',
        description: `Necesitas ${formatEcoAmount(reward.eco_cost - balance)} $EC0 más para canjear esta recompensa`,
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      const { error } = await supabase
        ...
        .insert({
          ...
          eco_spent: selectedReward.eco_cost,
          status: 'completed',
        });
      ...
      toast({
        ...
        description: `Has canjeado ${selectedReward.name} por ${formatEcoAmount(selectedReward.eco_cost)} $EC0`,
      });
        ...
        <div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {nftRewards.map((reward) => {
              const canAfford = balance >= reward.eco_cost;
              return (
                <Card
                  key={reward.id}
                ...
                >
                  <CardContent className="p-0">
                    ...
                    <div className="p-6 space-y-4">
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-2xl font-bold text-purple-600">
                            {formatEcoAmount(reward.eco_cost)}
                          </p>
                          <p className="text-sm text-gray-500">$EC0</p>
                        </div>
                      ...
                      >
                        {canAfford ? 'Canjear' : 'Insuficiente $EC0'}
                      </Button>
                    </div>
                  </CardContent>
                </Card>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xl font-bold text-emerald-600">
                    {formatEcoAmount(selectedReward.eco_cost)} $EC0
                  </span>
                </div>
                <div className="flex items-center justify-between mt-2">
                  <span className="text-sm text-gray-600">Balance después:</span>
                  <span className="text-xl font-bold text-gray-700">
                    {formatEcoAmount(balance - selectedReward.eco_cost)} $EC0
                  </span>
                </div>
              </div>
